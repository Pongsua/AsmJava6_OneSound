        ...
        playButtons[i].addEventListener("click", () => {
            ...
            if (songId) {
                ...
                fetch(songUrl)
                    ...
                    .catch((error) => {
                        console.error(
                            "Lỗi khi lấy dữ liệu hoặc phân tích JSON:",
                            error
                        );
                    });
            }
  ...
  function renderPlaylists(playlists){
    ...
    Array.from(updateButtons).forEach((a) => {
      a.addEventListener("click", (event) => {
        ...
        const targetId = event.target.closest('a[data-toggle="tooltip"][ title="Update"]')
...
function attachAddButtonEvents(songs) {
  ...
  function displayAddToPlaylistModal(songId) {
    ...
    const playlistItems = playlistDropdown.getElementsByClassName("playlist-item");
